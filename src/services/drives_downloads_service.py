...
class DrivesDownloadService:
    ...
    def __download_drive(self, drive_uuid: str, user: User, queue: Queue) -> BinaryIO:
        drive_download_props: DriveDownloadServiceProps = DriveDownloadServiceProps(user, drive_uuid)

        drive_download_service: IService[DriveDownloadServiceProps, Tuple[BinaryIO, str]] = DriveDownloadService()
